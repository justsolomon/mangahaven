...
class SignUp extends React.Component {
	...
	registerUser = (e) => {
		...
		fetch('https://mangahaven-server.netlify.app/.netlify/functions/app/users/register', {
			method: 'POST',
			headers: {
				...
				if (data._id !== undefined) {
					...
					localForage.getItem('user')
						.then(value => {
							...
							localForage.setItem('user', value)
								.then(value => {
									console.log(value);
					...
					else if (data.email) this.setState({ errorMsg: 'Email address already exists' });
					else if (data.passwordTwo) this.setState({ errorMsg: 'The passwords you entered do not match' });
				}
			})
			.catch(err => {
				...
				this.setState({
					loading: false,
					errorMsg: 'An error occurred. Please check your internet connection and try again'
				})
			})
	}

	logUserIn = (email, password) => {
		fetch('https://mangahaven-server.netlify.app/.netlify/functions/app/users/login', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
